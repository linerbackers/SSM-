...
<body>
<div class="pd-20">
    <form action="<%=ctxPath%>/page/user/list.do">
        ...
        <div class="text-c"> 
            ...
            <input type="text" name="param" value="${param.param }" placeholder=" 学生姓名、学校、邮箱、手机等" style="width:250px" class="input-text" />
            ...
            <tbody>
                <c:forEach items="${pageInfo.list }" var="user">
                    <tr>
                        ...
                        <td>${user.phone }</td>
                        <td>
                            <button class="btn size-MINI radius" onclick="showLayer('修改用户','<%=ctxPath %>/page/user/update.do?id=${user.id }')">修改</button>
                            <button class="btn size-MINI radius" onclick="ajaxSubmit('<%=ctxPath%>/page/user/isTeacher','id=${user.id }')">
                            <c:choose>
                            <c:when  test="${user.isTeacher==true }">
                            	设为学生
                            </c:when>
                            <c:when test="${user.isTeacher==false }">
                            	设为老师
                            </c:when>
                            </c:choose>
                             </button>
