...
<script type="text/javascript">
    ...
    function selectSubject(selectDom){
    	location.href="<%=ctxPath%>/page/cardSubject/order.do?subjectId="+selectDom.value+"&t="+new Date().getTime();
    }
</script>
    <div class="pd-20">
        <form onsubmit="saveOrders(this)" action="<%=ctxPath%>/page/cardSubject/order.do">
                        ...
                        <c:forEach items="${cardList }" var="card">
                            <c:if test="${cardSubject.cardId==card.id }">
                                <tr id="tr${card.id }">
                                    ...
                                    <td>
                                        <a class="btn size-MINI" onclick="$('#tr${card.id }').insertBefore($('#tr${card.id }').prev());"><i class="Hui-iconfont"></i></a>
                                        <span style="margin:0 10px;">${cardSubject.seqNum }</span>
                                        <a class="btn size-MINI" onclick="$('#tr${card.id }').insertAfter($('#tr${card.id }').next());"><i class="Hui-iconfont"></i></a>
                                    </td>
                                    <td>${card.name }</td>
                                </tr>
                            </c:if>
